...
| Space | 대시 (대시 중 무적) |
| F | 바닥 드랍 획득 / 마을 NPC 대화 |
| R (누르고 있기) | 무기 교체 링 메뉴 (첫 보스 이후 해금) |
| P | 일시정지 메뉴 |

한 판은 **검 1종**으로 시작합니다. 오른손은 비어 있고 콤보스킬도 잠겨 있어, 처음에는
검 기본 공격만 나갑니다. 기본 공격이 5번 명중하면 그 무기가 각성해 콤보스킬로 공격이
바뀌고, 5초 안에 다시 맞히지 못하면 풀립니다.

보스를 잡으면 보상이 **바닥에 떨어집니다.** 가까이 가서 `F`로 주워야 얻고, 줍기 전에는
출구가 열리지 않습니다. 그 다음 마을 방에서 NPC에게 `F`로 말을 걸어 무기별 콤보스킬과
보조형스킬을 배치합니다.
...
현재 게임 내 이미지와 사운드는 전부 코드로 생성하며, 외부 에셋을 사용하지 않습니다.
