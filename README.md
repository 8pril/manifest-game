...
**콤보는 기본 규칙이 아니라 고르는 것입니다.** 첫 보스가 주는 보조2형 `콤보 개방`을
마을에서 무기에 붙이면, 그 무기만 기본 공격 5회 명중 시 강화기술로 전환됩니다.
5초 안에 다시 맞히지 못하면 풀립니다. 붙이지 않은 무기는 계속 기본 공격만 씁니다.
...
http://localhost:5173/?combo=1                 콤보 개방을 미리 장착 (콤보 빌드)
http://localhost:5173/?scene=Play              타이틀을 건너뛴다
```

`combo=1`은 콤보 유무를 비교할 때 씁니다. 콤보는 첫 보스가 주는 보조2형을 마을에서
