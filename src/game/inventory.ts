 ...
 * - 버리거나 파괴할 수 없다
 *
 * **버릴 수 없는데 페이지도 없다.** 성립하는 이유는 게임에 있는 것을 다 모아도
 * 30개라 49칸에 들어가기 때문이다. 넘치면 `reconcileLayout`이 조용히 건너뛰어
 * 보유 목록에는 있는데 격자에 안 보이는 항목이 생긴다. 그 전제는
 * `inventory-capacity.test.ts`가 지킨다 — 깨지면 칸을 늘리거나 페이지를 넣을 때다.
 * - 하단 버튼을 누르면 자동정렬된다
 * - 필터 3종: 전체 / 무기만 / 보조형스킬만
 *
